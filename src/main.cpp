...
static void printUsage(const char* prog)
{
    std::cout << "用法: " << prog << " <视频路径> [选项]\n"
              ...
              << "  --tau-min F          ViBe最低置信阈值 (默认 0.02)\n"
              << "  --tau-sigma F        局部标准差归一化阈值 (默认 45)\n"
              << "  --flow-scale F       光流下采样比例 (默认 0.5)\n"
              << "  --shadow-sat-max F   阴影抑制饱和度阈值 (默认 35)\n"
              << "  --shadow-val-ratio F 阴影抑制亮度比值 (默认 0.55)\n"
              << "  --temporal-window N  时序滤波窗口帧数 (默认 3)\n"
              << "  --temporal-votes N   时序滤波最少投票数 (默认 2)\n"
              << "  --ghost-flow-thresh F 鬼影抑制静止判定阈值 (默认 0.8)\n"
              << "  --ghost-max-frames N 鬼影抑制最大连续静止帧 (默认 5)\n"
              << "  --erode-size N       边界腐蚀大小 (默认 0)\n"
              << "  --flow-warp-weight F 光流前推融合权重 (默认 0.25)\n"
              << "  --adapt-r-min N      自适应R下限 (默认 15)\n"
              << "  --adapt-r-max N      自适应R上限 (默认 40)\n"
              << "  --morph-ksize N      形态学核大小 (默认 3)\n\n"
              << "按键控制:\n"
              << "  ESC / q      退出\n"
              << "  SPACE        暂停/继续\n"
              << "  s            保存当前帧 Mask 与叠加图\n"
...
int main(int argc, char** argv)
{
    ...
    for (int i = 2; i < argc; i++) {
        ...
        if (arg == "--show-fusion") {
            ...
            params.onnxPath = argv[++i];
        } else if (arg == "--refine-onnx") {
            params.refineOnnxPath = argv[++i];
        } else if (arg == "--dump-intermediates") {
            params.dumpIntermediates = true;
        } else if (arg == "--no-persistence") {
            params.temporalPersistence = false;
        } else if (arg == "--no-edge-fill") {
            params.edgeFill = false;
        } else if (arg == "--no-frame-diff") {
            params.frameDiffFill = false;
        } else if (arg == "--no-dist-fill") {
            params.distTransformFill = false;
        } else if (arg == "--dump-refinement") {
            params.dumpRefinement = true;
        } else if (i + 1 < argc) {
            ...
            else if (arg == "--tau-min")      params.tau_min = std::stod(val);
            else if (arg == "--tau-sigma")    params.tau_sigma = std::stod(val);
            else if (arg == "--flow-scale")   params.flowScale = std::stod(val);
            else if (arg == "--shadow-sat-max")  params.shadowSatMax = std::stod(val);
            else if (arg == "--shadow-val-ratio") params.shadowValRatio = std::stod(val);
            else if (arg == "--temporal-window")  params.temporalWindow = std::stoi(val);
            else if (arg == "--temporal-votes")   params.temporalVotes = std::stoi(val);
            else if (arg == "--ghost-flow-thresh") params.ghostFlowThresh = std::stod(val);
            else if (arg == "--ghost-max-frames")  params.ghostMaxFrames = std::stoi(val);
            else if (arg == "--erode-size")     params.erodeSize = std::stoi(val);
            else if (arg == "--flow-warp-weight") params.flowWarpWeight = std::stod(val);
            else if (arg == "--adapt-r-min")    params.adaptRMin = std::stoi(val);
            else if (arg == "--adapt-r-max")    params.adaptRMax = std::stoi(val);
            else if (arg == "--morph-ksize")    params.morphKernelSize = std::stoi(val);
            else if (arg == "--dump-dir")       params.dumpDir = val;
            else if (arg == "--refinement-dir") params.refinementDir = val;
            else if (arg == "--max-dim")       params.maxDim = std::stoi(val);
            else if (arg == "--flow-gate")      params.flowGateThresh = std::stod(val);
            else if (arg == "--persist-frames") params.persistenceFrames = std::stoi(val);
    ...
    if (!params.onnxPath.empty())
        detector.loadONNX(params.onnxPath);
    if (!params.refineOnnxPath.empty())
        detector.loadRefineONNX(params.refineOnnxPath);
    ...
    if (params.dumpIntermediates && !params.dumpDir.empty())
        mkdir(params.dumpDir.c_str(), 0755);
    if (params.dumpRefinement && !params.refinementDir.empty())
        mkdir(params.refinementDir.c_str(), 0755);
